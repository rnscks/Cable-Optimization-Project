...
class JumpPointSearch(GridAlgorithm):
    ...
    def search(self):
        ...
        while self.open_list:
            ...
            for direction in self.scan_directions:
                jump_node = self.jump(cur_node, direction.dir_i, direction.dir_j, direction.dir_k)   
                
                if isinstance(jump_node, Node):
                    if jump_node.is_goal_node == True:
                        jump_node.parent = cur_node
                        return True
                    
                    ng: float = cur_node.g + cur_node.center_pnt.Distance(jump_node.center_pnt)
                    if jump_node.parent is None or ng < jump_node.g:
                        jump_node.g = ng
                        jump_node.parent = cur_node
                        jump_node.f = jump_node.g + jump_node.center_pnt.Distance(self.goal_node.center_pnt)    
                        heapq.heappush(self.open_list, jump_node)                         
    ...
    def jump(self, cur_node: Node, dir_i: int, dir_j: int, dir_k: int) -> Optional[Node]:     
        scaned_node_set = set() 
        ...
        elif dir_i != 0 and dir_j != 0 and dir_k == 0:
            is_where = 4
        ...
        while True:
            ...
            if nxt_i < 0 or nxt_i >= map_size or nxt_j < 0 or nxt_j >= map_size or nxt_k < 0 or nxt_k >= map_size:
                self.close_list.update(scaned_node_set)
                return None
            ...
            if nxt_node.is_goal_node == True:
                return nxt_node
            if self.has_forced_neighbor(nxt_node, dir_i, dir_j, dir_k):
                self.close_list.update(scaned_node_set)    
                return nxt_node
            
            if is_where != 1:
                if is_where == 3:
                    jump_directions = [(dir_i, 0, 0), (0, dir_j, 0), (0, 0, dir_k),
                                        (dir_i, dir_j, 0), (dir_i, 0, dir_k), (0, dir_j, dir_k)]
                elif is_where == 4:
                    jump_directions = [(dir_i, 0, 0), (0, dir_j, 0)]
                elif is_where == 5:
                    jump_directions = [(0, dir_j, 0), (0, 0, dir_k)]    
                else:
                    jump_directions = [(dir_i, 0, 0), (0, 0, dir_k)]
                
                for jump_direction in jump_directions:
                    if self.jump(nxt_node, jump_direction[0], jump_direction[1], jump_direction[2]) != None:
                        return nxt_node
                    else:
                        self.close_list.update(scaned_node_set) 
                            
            cur_i, cur_j, cur_k = nxt_i, nxt_j, nxt_k   
            scaned_node_set.add(nxt_node)   
            ...
            for i in range(-1, 2):
                for j in range(-1, 2):
                    for k in range(-1 ,2):
                        ...
                        if nei_i < 0 or nei_i < 0 or nei_i < 0:    
                            continue
